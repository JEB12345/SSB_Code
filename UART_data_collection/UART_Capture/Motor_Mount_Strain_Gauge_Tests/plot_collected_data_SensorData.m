...
torque = (namesOfFiles.*9.81./1000.*0.0575);
extrapulatedTorque = (0:200:20000).*9.81./1000.*0.0575;
[p,ErrorEst] = polyfit(torque,avgDataPoints,2);
[fit_plot,delta] = polyval(p,extrapulatedTorque,ErrorEst);
h=figure;
plot(extrapulatedTorque,fit_plot,'-',torque,avgDataPoints,'+',extrapulatedTorque,fit_plot+2*delta,'r:',extrapulatedTorque,fit_plot-2*delta,'r:');
legend('Polynomial Model','Data','95% Confidence');
xlabel('Ideal N-m Point Values');
